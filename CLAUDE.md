...
- 详情页「🎪 展台活动」「✅ 展台任务」两张卡的标题行是 `.fold-head`（整行可点），右上角 `.linkbtn.fold-tri` 三角 ▾ / ▸ 收起展开，**默认展开**，换展位（`props.id` 变化）时复位为展开。`.fold-tri` 必须写成 `.linkbtn.fold-tri`：`.linkbtn` 在 `style.css` 里更靠后，同为单类选择器会盖掉颜色和下划线。
