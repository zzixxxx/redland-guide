...
- 四个底部 Tab：
  ...
  2. **花车巡礼**（`/parade`）：打卡 / 巡游时间 → 官方花车巡礼路线图卡（可折叠，默认展开）→ 「头号花车 / 专属花车」两张贴纸当 tab + DAY1–5 芯片（两边共用）→ 头号花车当日出场角色 或 8 台 IP 专属花车（效果图 + 按当前 DAY 的出席嘉宾名单 + 笔记图）→ 主角方阵 → 页尾「📎 来源」卡（可折叠，默认展开，全页来源集中在这里，卡片里不再各写一行）。
...
- **花车页**：黄色 `.theme-banner` 横条已删（用户 9/17：没啥用）；「头号花车 / 专属花车」是两颗 `.sticker.tab` 按钮，未选中加 `.off`（纸色）；`tab` 只存组件内。舞台页的 `.theme-banner.moon` 保留。底栏 `TabBar` 四个图标：🗺️ 展位攻略 / 🎠 花车巡礼 / 🎤 月光舞台 / 💾 PIN 图鉴（用户 9/17 把 📀 换成 💾）。
