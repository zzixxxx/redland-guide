...
- **平面图「地图模式」**（用户 9/14）：灯箱 item 带 `spots`（= `mapSpots.js` 的 `mapSpotList`）时走这套，只有官方平面图 P2 用，别的灯箱走原来的裸 `img` 分支不受影响。
  - 打开时整图 contain 进舞台（**不要再用 `.wide` 铺满高度**，用户 9/14：按图片大小缩放）；缩放平移由组件自己用 `transform` 做（`.lb-stage.map` 上 `touch-action:none`），双指缩放 / 单指拖动 / 滚轮 / 底栏 －＋ 都走同一套 `zoomBy`。**iOS Safari 的双指缩放必须在 `gesturestart/gesturechange/gestureend` 上 `preventDefault`**，否则会把整页放大、盖掉图里的缩放（用户 9/14 明确要求优先图里的功能）。
  - 单击热区选中 + 弹 `.lb-bub` 气泡；**气泡里直接点 IP 名字跳攻略，不要再写「攻略 ›」**（用户 9/14）；只有一个 IP 的展位还可以双击直接跳，多 IP 的双击只做选中。有选中时点空白先取消选中、不关灯箱。跳转用 `@open="id"` 交回页面（`BoothsPage.openBooth`），不在组件里写路由。
  - 气泡里的「🧭 导航」在图上画一条起点 → 展位的路线：`utils/route.js` 在 `walkGrid` 上跑 A*，展位格加通行代价（不是挖掉，挖了会切断窄过道导致无解），路线因此走展位之间的过道。起点 `mapStart` = 安检票检区。
...
- [ ] 平面图展位热区：**A 区 40 个已做完**（9/14 试点，`mapSpots.js` + `scripts/map-spots.py`，点展位 → 气泡跳攻略 / 导航），B / C 区待补——跑 `python scripts/map-spots.py B`，看 `cand_B.png` 把序号→展位号填进脚本 LABELS 再 `--emit`；C 区配色是蓝灰系，脚本里的描边阈值要另调
