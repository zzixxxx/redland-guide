...
- [ ] A17b 归环：奖品里的「PIN 卡」是否为冒险者拼图用的区域 PIN 待确认（外观是卡片不是软盘，确认后补进 `pins.js`）；女仆迎宾名单与场次、花车巡礼出场角色待公布
