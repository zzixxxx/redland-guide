...
- `mapGates` 是固化下来的出入口修正（`[{ x, y, r, mode }]`，默认空数组）、`mapStrokes` 是固化下来的画笔笔迹（`[{ mode, r, pts }]`，`pts` 是路网格坐标；9/17 起有 3 笔：B 区内部横向廊道 gy 82 从西侧竖廊 gx 137 到 B02/B01 之间竖廊 gx 222，以及 B07↔B06、B02↔B01 两个南侧出入口的竖连接——官方图上这条廊道没画成实线，路网里全是桥接格 ×6，从冒险者营地一侧去 B01 时 A* 宁可绕 B 区外圈实线走 1218px，铺成真线后 850px、并沿 B07→B06→B05→B04→B03→B02/B01 走；调试方法见 §5「路网调试」），和本机覆盖一起在 `stampGates()` 里盖到路网上；`emptyDev()` 的字段是 `spots / doors / gates / strokes / start / removed`。地图下方常显一行图例（`.dev-legend`：青 = 真线、紫 = 桥接格 ×6、画笔铺 / 封、试走路线），用户 9/17 曾把桥接格的紫红看成红色，所以桥接格改成偏蓝的紫 `rgb(170,60,255)`、画笔封路才用红。
