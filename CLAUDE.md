...
- **详情页顶部「官方一句话」卡**（用户 9/17）：标签是红「A06」+ 蓝「A 区 · 翻身时空港」（区域名取 `zones[].region`）——页头副标题已经是「A 区 · 展位 A06」，标签只写「A 区」就重复了。按钮行第一颗「🧭 地图上导航到 A06」：打开官方平面图 P2 灯箱（与首页同一套 `mapSlices`）并传 `navTo`，灯箱打开带 `spots` 的图时对该展位号自动跑一遍 `nav()` 画起点 → 展位路线；气泡里点别的 IP 由 `@open` 关灯箱并 `router.push('/booth/<id>')`。`mapNo` = id 去掉 a/b/c 后缀（A25a → A25），再退回官方展位号原文各段（B02 / B17）取图上有热区的那个；拿不到的展位（待解锁 / 无热区）不显示「导航」与「加清单」。`Lightbox` 的 `navTo` prop 只在 item 带 `spots` 时生效，其他灯箱不受影响。
