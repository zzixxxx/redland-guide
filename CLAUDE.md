...
| `parade.js` | `paradeInfo` / `paradeRoute`（官方路线图：`image` / `desc` / `route` 按图理解的走法 / `legend` / `encounters` / `source`）/ `paradeDays[{ day, date, entries[{ ip, chars[] }] }]` / `themeFloats[{ id, ip, ips?, desc, look, intro, images[], guests[{ day: 1–5 \| 'all', label?, chars[] }], guestNote?, source }]`（`ip` 用短名：逆水寒 / 永劫无间 / 无畏契约 / 米哈游；融合花车用 `ips` 列子 IP，灰字显示在抠图下方并参与详情页跳转匹配；`desc` 保留官方文案但页面不显示；每台另有 `public/img/parade/<id>/cut.png` 透明底抠图，用 `scripts/cut-floats.py`（rembg）从 00.jpg 抠出） / `playerSquad` | 官方「花车巡礼」半层 + RED LAND 官方号 9/11 路线图笔记 + 9/5 七条「前方高能！XX 专属花车准备发车！」笔记 + RED LAND 官方号 9/16「前方高能！米哈游四大IP花车准备出发！」（第 8 台 `mihoyo`，星布谷地 9/16 自己预告的「奥陌陌专属花车」已并入这一台、不再单列（用户 9/17），星布谷地那条笔记仍在 A06 详情与 stage.js 里：原神 派蒙 / 星穹铁道 帕姆 / 绝区零 伊埃斯 / 星布谷地 奥陌陌，官方未标日期按 `day: 'all'`；`ip` 写成「米哈游 · 原神 / 崩坏：星穹铁道 / 绝区零 / 星布谷地」，`utils/ipMatch.js` 按斜杠拆开后四个展位都能对上） |
...
| `rules.js` | `event`（含 `days[]`）/ `mainline`（含 `regions[].pin/color`、`nightPin`、`images`、`source`）/ `eggs` / `places` / `dailySchedule` / `venueMap`（官方平面图：`images` / `routes` 路线图例 / `routeTip` / `tips` 交通与点位 / `facilities` 回血点位 / `source`）/ `venueFacilities`（功能点位指南：`groups[{ title, items[] }]` / `note` / `images` 五段放大标注图 / `source`）/ `venueMapRef`（2025 年参考图 + 交通要点，非官方，保留）/ `equipPack`（主角专属装备包：`items[{ name, desc, note }]` 五件 + `notes` + 7 张图 + `source`；首页「主线玩法」卡下方「🎒 主角专属装备包」折叠卡，默认收起、标题下常显一句领取方式；RED LAND 官方号 9/15，入场安检后到装备区人人可领，不涉票务） | 官方「冒险者攻略」半层 + 主会场 + RED LAND 官方号 8/25「PIN 收集玩法」、9/11「登岛地图已解锁」「功能点位指南」笔记；`venueMapRef` 来自网友「星辰大海」2025-08-07 笔记 |
...
- 花车类官方笔记（路线图、「XX 专属花车准备发车」）进 `parade.js`，图放 `public/img/parade/<id>/`（id 用拼音 / 英文简称）。专属花车的「出席嘉宾角色名单」按官方图逐字进 `themeFloats[].guests`，按日的写 `day: 1–5`，写「DAY1–DAY5」的写 `day: 'all'`；官方图上的分段标题（如逆水寒「特别开场：神秘惊喜亮相」）放 `label`。花车页按当前 DAY 显示；**专属花车的出席嘉宾名单用普通黑色小字顿号拼接（`.sched-guests`），不套 `.pill` 胶囊**（用户 9/17），头号花车那栏的角色胶囊不变。
...
- **花车页**：黄色 `.theme-banner` 横条已删（用户 9/17：没啥用）；「头号花车 / 专属花车」是两颗 `.sticker.tab` 按钮，未选中加 `.off`（纸色）；`tab` 只存组件内。**专属花车 tab 与头号花车同款 `.pr-entry` 行式布局**（左栏：IP 短名 → `.fl-cut` 透明底抠图（点开效果图）→ 融合花车子 IP 灰字 `.fl-sub` → 🚗 造型；右栏 `.chars.fl-body` 竖排：固定标题「出席嘉宾」→ 按所选 DAY 顿号拼接名单（没有当日名单写「当日名单待公布」）→ 官方分段标题 `label` 灰字 → 备注 → `.fl-thumbs` 56px 缩略图一排，点开灯箱），**不显示 `desc` 一句话文案、标题里不带 DAY 日期**（用户 9/17），不要再用大图 `.gallery`。舞台页的 `.theme-banner.moon` 保留。底栏 `TabBar` 四个图标：🗺️ 展位攻略 / 🎠 花车巡礼 / 🎤 月光舞台 / 💾 PIN 图鉴（用户 9/17 把 📀 换成 💾）。
...
- [ ] 米哈游专属花车（第 8 台 `mihoyo`，星布谷地那台已并入）：官方嘉宾名单未标日期，按全程记；巡游出场时间待节目单。抠图 `cut.png` 是 rembg 自动抠的，阅文 / 米哈游两台结构细碎，效果一般，官方出实拍后可重抠。拳头游戏音乐 DJ 专场曲目只公布了 TICKING AWAY / 2 WORLDS「等」，完整歌单待现场
