...
已收录：`A06` 星布谷地；`B02`（B-02 / B-17）与 `C16` 宝可梦（一条笔记覆盖三个展位，两个 key 共用同一对象，只改 `boothNo`）；`A09` 崩坏：星穹铁道（笔记主体是线上征集，只收录 RED LAND 参展情报两张图）。其余 IP 详情按 §6 流程补。
...
- [ ] 其余 IP 的展台详情（已收录 A06 星布谷地、B02 / C16 宝可梦、A09 星穹铁道）
