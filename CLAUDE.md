...
src/
  ...
  composables/useStore.js     useChecked（展位打卡，rl26.checked）/ useDay（花车与舞台共享的当前 DAY，rl26.day）/ useCollected（PIN 已收集，rl26.pins）/ usePlan（待打卡清单，rl26.plan，按展位号 no 存）
  ...
  utils/plan.js               待打卡清单排路线：mapSeq 蛇形序当种子 + 2-opt（见 §5）；fmtDist 按官方「200M」箭头定的比例尺换算米数
...
| `mapSpots.js` | `mapSeq` 逛展动线序号（蛇形序，路线种子与列表排序用，**不替代官方编号**）；`mapDoors` + `doorPoint()` 每个展位的到达门（A 左 / B 上 / C 右 / D 下，朝向过道那条边，**导航终点用它、不要用中心点**）；`mapSpots`：81 个展位号 → **P2 切片**上的归一化热区 `[x, y, w, h]`（p2.jpg 与 p2-full.jpg 共用）；`mapStart` 登岛起点；`walkGrid` 300×217 路网；`mapFacilities` 功能点位（遮罩点亮用）；`mapSpotList` 再附该展位号下的全部 IP | `scripts/map-spots.py`：`<区> --emit` 出热区、`--walk` 出路网，序号→展位号人工核对。**热区坐标最后是直接在 `p2-full.jpg`（6000×4344）上重测的**——先经 01/02/03.jpg → 概览图 → 整图 → P2 转换会累积几像素误差，看着就是框整体偏右下（用户 9/14） |
...
- **平面图「地图模式」**（用户 9/14）：灯箱 item 带 `spots`（= `mapSpots.js` 的 `mapSpotList`）时走这套，只有官方平面图 P2 用，别的灯箱走原来的裸 `img` 分支不受影响。
  ...
  - **待打卡清单**（用户 9/15）：`usePlan` 按展位号存在 `rl26.plan`；首页展位行的 `＋` 与详情页「＋ 加清单」都能加。清单卡按 `planRoute()` 排好顺序、列出每段距离，「在地图上看路线」把整条多点路线传给 `Lightbox` 的 `plan` prop，图上画连线 + 编号站点（`.lb-stop`）。排序用 `mapSeq` 当种子再跑 2-opt，距离矩阵用 `findDistances`；12 个点约 50ms、81 个约 370ms。
  - **气泡布局**：展位号 → IP 名（每行 ≥34px）→ 「导航到这里」全宽按钮独占底部。IP 跳转和导航是两种动作，必须分开放且给足行高，否则手指容易点串（用户 9/15）；按钮里不要放 emoji。按下导航后**自动收起气泡**（不然挡着看不到路线），点空白依次是：收气泡 → 清路线 → 关灯箱。
