...
- **`stage[].schedule[].time`**（可选）：场次多的展台（宝可梦）排上它——时段整段写一行（如 `'13:30 – 14:30、15:15 – 17:30'`），下面的 `guests` 改用黑色小字顿号分隔（`.sched-time / .sched-guests`），不再是胶囊；标题后要跟上明确的展位点（如「（B-17 主会场 · 舞台处）」）。**没排 `time` 的展台保持原样**（用户 9/11 要求保留胶囊，别全局改）。
