...
已收录：`A06` 星布谷地；`B02`（B-02 / B-17）与 `C16` 宝可梦（一条笔记覆盖三个展位，两个 key 共用同一对象，只改 `boothNo`）；`A09` 崩坏：星穹铁道（笔记主体是线上征集，只收录 RED LAND 参展情报两张图）；`B16` 宝藏码头（RED LAND 官方号发布，集市型，用 `hours / location / stalls`）。其余 IP 详情按 §6 流程补。
...
- [ ] A / B / C 区 ↔ 翻身时空港 / 重生试炼场 / 黄金海岸线 的映射（影响主线玩法的区域进度口径，目前按 A/B/C 计数）。已知线索：官方称 B16 宝藏码头位于「黄金海岸线」，B08 遗忘之海文案提到「海岸线」→ B 区很可能是黄金海岸线，待官方平面图确认。
- [ ] 夜间「月下模式」具体开启时刻、9 月底「活动预约」入口
- [ ] 其余 IP 的展台详情（已收录 A06 星布谷地、B02 / C16 宝可梦、A09 星穹铁道、B16 宝藏码头）
