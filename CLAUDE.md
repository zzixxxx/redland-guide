...
| `mapSpots.js` | `mapSpots`：展位号 → **P2 切片**上的归一化热区 `[x, y, w, h]`（p2.jpg 与 p2-full.jpg 共用）；`mapStart` 登岛起点（安检票检区）；`walkGrid` 180×130 可走网格（导航寻路用）；`mapSpotList` 再附该展位号下的全部 IP | `scripts/map-spots.py`：`A --emit` 出热区、`--walk` 出可走网格，序号→展位号人工核对 |
...
- **平面图「地图模式」**（用户 9/14）：灯箱 item 带 `spots`（= `mapSpots.js` 的 `mapSpotList`）时走这套，只有官方平面图 P2 用，别的灯箱走原来的裸 `img` 分支不受影响。
  ...
  - 气泡里的「🧭 导航」在图上画一条起点 → 展位的路线：`utils/route.js` 在 `walkGrid` 上跑 A*。**`walkGrid` 是官方图上画出来的线本身**（冒险主线红 / 花车路线橙 / 冒险支线蓝 / 区域内红色细虚线粉），不是空地——用户 9/14：每块区域都有严格的出入口，不能随意穿插展位。300×217 最大池化 + 5×5 膨胀连上虚线断点与线线交汇，再取起点所在连通域（61 个展位都能接上）。海水也是蓝的，靠 `r<60` 与支线蓝区分。起点 `mapStart` = 冒险者大道上「注意花车出没哦!」前面的红色手柄机。
...
- [ ] 平面图展位热区：**A 区 40 个 + B 区 21 个已做完**；**C 区待补**——官方图上 C 区的展位是 2–4 个一组挤在同一个大蓝框里，组内分隔线太浅，自动识别（试过 4 组阈值 + 行列投影切分）只能拆到「组」这一级，得对着 `cand_C.png` 逐个手量框坐标填进 `scripts/map-spots.py` 的 MANUAL
- [ ] 选中功能点位时像功能点位指南那样遮罩点亮全部同类点位（用户 9/14 提出）：要先把医疗点 / 卫生间 / 充电宝 / 餐饮 / 寄存处 / 换装区 / 结算点 / 服务台在 P2 上的坐标逐个量出来，目前只有展位热区
