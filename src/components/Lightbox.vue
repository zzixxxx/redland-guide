<template>
  <Teleport to="body">
    <div v-if="cur" class="lightbox" @click="close">
      <div ref="stage" class="lb-stage" :class="{ tall }" @touchstart.passive="onTouchStart" @touchend="onTouchEnd" @click.stop="onStageClick">
        <img :key="cur.src" :src="cur.src" :alt="cur.caption || ''" @load="onLoad" />
      </div>
...
watch(() => props.index, () => (tall.value = false))

function onLoad(e) {
  const img = e.target
  const st = stage.value
  if (!st || !img.naturalWidth || !st.clientWidth) return
  tall.value = img.naturalHeight / img.naturalWidth > (st.clientHeight / st.clientWidth) * 1.2
}
